...
import { THREED_ELEMENTS_LIST } from 'src/data/graphics/threeDElementsList';

const { Search } = Input;
// onSelect is passed from background of workspace
function ThreeDElements({ onSelect = null, canvas, updateLocalCanvas }) {
    const { token } = theme.useToken();
    const [activeGrCategory, setActiveGrCategory] = useState(Object.keys(THREED_ELEMENTS_LIST)[0])
        ...
        if (onSelect) {
            ...
            if (iconType == 'svg') {
                fabric.loadSVGFromURL(icon, (objects, options) => {
                    ...
                    updateLocalCanvas(canvas, 'ThreeDElements')
                });
            } else {
    ...
    const onChangeSearchQuery = (query) => {
        ...
        THREED_ELEMENTS_LIST[activeGrCategory].map((catgeory) => {
            ...
            {Boolean(selectedCategoryToDisplay) ? <>
                ...
                <div className={`${styles.iconsListWrap}`} style={{ background: token.colorBgBase }}>
                    <div className={`${styles.stickyCategory} ${styles.graphicsGroupCat}`} style={{ background: token.colorBgBase }}>
                        {Object.keys(THREED_ELEMENTS_LIST).map((grCategory: any) => {
                    ...
                    {Boolean(searchQuery || activeGrCategory) && <div className={`${styles.stickyCategory} ${styles.groupCategoryList}`} style={{ background: token.colorBgBase }}>
                        <div className={styles.categoriesWrap}>
                            {(searchQuery ? filteredCategory : THREED_ELEMENTS_LIST[activeGrCategory]).map((category, i) => {
